...
def get_net():
    ...
    net._fc = nn.Linear(in_features=net._fc.in_features, out_features=2, bias=True).to('cuda:1')
    return net
...
#             model = nn.DataParallel(model)
    ...
    def net_forward(net, inputs):
        bs = inputs.size(0)
        # Convolution layers
        x = net.module.extract_features(inputs)
        # Pooling and final linear layer
        x = net.module._avg_pooling(x)
        emb = x.view(bs, -1)
        x = net.module._dropout(emb)
        x = net.module._fc(x)
        return emb, x
...
print(result)
print("--- %s hours ---" % ((time.process_time())/3600))
